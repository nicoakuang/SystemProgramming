...
pthread_mutex_t mutex1 = PTHREAD_MUTEX_INITIALIZER; // Initialize a mutex to control access to shared variables
...
{
  for (int i = 0; i < LOOPS; i++) {  // Loop for the defined number of iterations
    pthread_mutex_lock(&mutex1);     // Lock the mutex to ensure exclusive access to money
    money = money + 1;               // Increment money by 1
    pthread_mutex_unlock(&mutex1);   // Unlock the mutex to allow other threads to access money
  }
